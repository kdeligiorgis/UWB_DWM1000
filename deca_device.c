...
void dwt_setlnapamode(int lna_pa)
{
    ...
    //gpio_mode &= ~(GPIO_MSGP4_MASK | GPIO_MSGP5_MASK | GPIO_MSGP6_MASK); /* my GPIO4 is not connected. KOSTAS. */
    ...
    {
        //gpio_mode |= (GPIO_PIN5_EXTTXE | GPIO_PIN4_EXTPA); /* my GPIO4 is not connected. KOSTAS. */
...
{
    return ((dwt_read32bitoffsetreg(GPIO_CTRL_ID, GPIO_RAW_OFFSET) & gpioNum)? 1 : 0);  // If Condition is true ? then value X : otherwise value Y
}
